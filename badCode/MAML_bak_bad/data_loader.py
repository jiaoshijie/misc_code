...
def genDataDict(inputdir, way=5, shot=1, train=True):
    ...
    for _ in tqdm(range(num_total_batches)):
        ...
        datadict.append({"s": support, "sl": support_l, "q": query, "ql": query_l})
...
class LoadDataset:
    ...
    def __getitem__(self, idx):
        # sl, ql = self.genlabels(self.datadict[idx]['sl']), self.genlabels(self.datadict[idx]['ql'])
        sl, ql = torch.LongTensor(self.datadict[idx]['sl']), torch.LongTensor(self.datadict[idx]['ql'])
        si, qi = self.pathToImg(self.datadict[idx]['s']), self.pathToImg(self.datadict[idx]['q'])
        si = torch.stack(si)
        qi = torch.stack(qi)
        return si, sl, qi, ql  # support set, support labels, query set, query labels
